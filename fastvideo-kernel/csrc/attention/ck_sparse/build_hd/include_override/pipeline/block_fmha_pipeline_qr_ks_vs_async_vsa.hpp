...
struct BlockFmhaPipelineQRKSVSAsyncVSA
{
               ...
               void* smem_ptr) const
    {
        ...
        auto s_acc_aux          = SaccBlockTileType{};
        ...
        // (AV-gemm dual-acc was tried — regressed because the per-iter clear
        // of o_acc_aux + the extra tile-add fold cost more than the
        // parallelism saved. AV's MFMA chain is shorter than QK's so the
        // upside is smaller; the bookkeeping cost dominated.)
        ...
        do
        {
            // STAGE 1, QK gemm — outer-K dual-accumulator.
            clear_tile(s_acc);
            clear_tile(s_acc_aux);
            ...
            __builtin_amdgcn_s_setprio(0);
            tile_elementwise_inout([](auto& a, auto b) { a = a + b; }, s_acc, s_acc_aux);
            __builtin_amdgcn_sched_barrier(0);
